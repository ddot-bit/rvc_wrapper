...
def cleanup_files(keep_files, is_webui, progress):
    # TODO: update logic
    if not keep_files:
        display_progress(
...
def preprocess_song(
    ...
    extract_audio_process = MdxProcess(
...
def voice_change(
    ...
    # TODO: centralized implementation
...
def song_cover_pipeline(
    ...
    try:
        ...
        if not os.path.exists(ai_vocals_path):
            ...
            voice_change(
                voice_model,
                # Only vocals is passed to rvc pipeline
                audio_out_path.main_vocals_dereverb_path,
                ai_vocals_path,
                pitch_change,
                f0_method,
                index_rate,
        ...
        if pitch_change_all != 0:
            ...
            audio_out_path.instrumentals_path = pitch_shift(
                audio_out_path.instrumentals_path, pitch_change_all
            )
            audio_out_path.backup_vocals_path = pitch_shift(
                audio_out_path.backup_vocals_path, pitch_change_all
            )
        ...
        combine_audio(
            [
                ...
                audio_out_path.backup_vocals_path,
                audio_out_path.instrumentals_path,
            ],
            ai_cover_path,
            main_gain,
            backup_gain,
...
if __name__ == "__main__":
    ...
    cover_path = song_cover_pipeline(
        song_input=args.song_input,  # can be youtube url
        voice_model=rvc_dirname,
        pitch_change=args.pitch_change,  # 1 == male to female, -1 == female to male
